...
namespace Main_Real_estate.English.Main_Application
{
    public partial class Edit_Contract : System.Web.UI.Page
    {
        ...
        //*****************************************************************************************************************
        //************************************ Contract Main Operations (Edit / Delete / Back ) *********************************************************
        protected void btn_Add_Contract_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                _sqlCon.Open();
                MySqlCommand cmd = new MySqlCommand("Edit_Single_Contract", _sqlCon);
                ...
                cmd.Parameters.AddWithValue("@units_Unit_ID", Units_DropDownList.SelectedValue);
                ...
                if (Real_Contract_FileUpload.HasFile)
                {
                    ...
                    cmd.Parameters.AddWithValue("@Real_Contract_FileName", fileName1);
                    cmd.Parameters.AddWithValue("@Real_Contract_Path", "/English/Main_Application/Real_Contract/" + fileName1);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@Real_Contract_FileName", Real_Contract_FileName.Text);
                    cmd.Parameters.AddWithValue("@Real_Contract_Path", Real_Contract_Path.Text);
                }

                cmd.ExecuteNonQuery();
                _sqlCon.Close();
                if (Session["Langues"].ToString() == "1") { lbl_Success_Add_New_Contract.Text = "Contract Edited Successfully"; }
                else { lbl_Success_Add_New_Contract.Text = "تمت تعديل العقد بنجاح"; }
            }
        }
        ...
        protected void Delete_Contract_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand getTransformationCmd = new MySqlCommand("SELECT Contract_Id FROM transformation_table WHERE Contract_Id = @ID And ( Status = 'غير محصل' or Status = 'Uncollected' )  ", _sqlCon);
            ...
            if (getTransformationDt.Rows.Count == 0)
            {
                ...
                MySqlCommand getCashCmd = new MySqlCommand("SELECT Contract_Id FROM cash_amount WHERE Contract_Id = @ID And ( Satuts = 'غير محصل' or Satuts = 'Uncollected' )", _sqlCon);
                ...
                if (getCashDt.Rows.Count == 0)
                {
                    ...
                    {
                        if (Session["Langues"].ToString() == "1") { Response.Write(@"<script language='javascript'>alert('This contract cannot be deleted because it contains uncollected cheques')</script>"); }
                        else { Response.Write(@"<script language='javascript'>alert('لا يمكن حذف هذه العقد لأنه يحتوي على  شيكات غير محصلة  ')</script>"); }
                            
                    };
                }
                else
                {
                    if (Session["Langues"].ToString() == "1") { Response.Write(@"<script language='javascript'>alert('This contract cannot be deleted because it contains uncollected Cashes')</script>"); }
                    else { Response.Write(@"<script language='javascript'>alert('لا يمكن حذف هذه العقد لأنه يحتوي على  دفعات غير محصلة  ')</script>"); }
                }
            }

            else
            {
                if (Session["Langues"].ToString() == "1") { Response.Write(@"<script language='javascript'>alert('This contract cannot be deleted because it contains uncollected Tranformations')</script>"); }
                else { Response.Write(@"<script language='javascript'>alert('لا يمكن حذف هذه العقد لأنه يحتوي على  حوالات غير محصلة  ')</script>"); }
            }
            _sqlCon.Close();
        }
        ...
        //************************************** SelectedIndexChanged Operations ******************************
        ...
        //-------------------------------------------   Paymen_Method_RadioButtonList_SelectedIndexChanged -------------------- 
        ...
        //-------------------------------------------   txt_No_Of_Months_Or_Years_TextChanged ----------------------------------
        ...
        //-------------------------------------------   txt_Duration_Of_The_Free_Period_TextChanged ----------------------------- 
        ...
        //-------------------------------------------   FREE_PERIOD_CheckBox_CheckedChanged ----------------------------- 
        ...
        //-------------------------------------------   Additional_Items_CheckBox_CheckedChanged ----------------------------- 
        ...
        {
            string contractId = Request.QueryString["Id"];
            string getCashsQuari = "select Csh.*, T.Tenants_Arabic_Name , T.Tenants_English_Name  FROM  cash_amount Csh left join tenants T on (Csh.tenant_Id = T.Tenants_ID) where Contract_Id = '" + contractId + "';";
