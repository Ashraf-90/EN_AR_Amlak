    ...
    public partial class Building_Condition_List : System.Web.UI.Page
    {
        ...
        protected void The_Table_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Header)
            {
                ...
                if (Dt.Rows.Count > 0)
                {
                    ...
                    {
                        lbl_1.Text = Dt.Rows[71]["EN"].ToString();
                        lbl_2.Text = Dt.Rows[70]["EN"].ToString();
                    }
                    else
                    {
                        lbl_1.Text = Dt.Rows[71]["AR"].ToString();
                        lbl_2.Text = Dt.Rows[70]["AR"].ToString();
                    }
                }
            }
        }
        ...
        protected void language()
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                if (Session["Langues"].ToString() == "1")
                {
                    lbl_titel.Text = Dt.Rows[264]["EN"].ToString();
                    ADD.Text = Dt.Rows[54]["EN"].ToString();
                }
                else
                {
                    lbl_titel.Text = Dt.Rows[264]["AR"].ToString();
