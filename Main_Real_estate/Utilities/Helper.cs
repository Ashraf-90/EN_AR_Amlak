...
namespace Main_Real_estate.Utilities
{
    public static class Helper
    {
        ...
        //******************  languge ***************************************************
        public static void Lan(MySqlConnection connection , string ID, string COl ,  Label label)
        {
            DataTable Dt = new DataTable();
            MySqlCommand Cmd = new MySqlCommand("SELECT * FROM languages WHERE ID = '"+ ID + "'", connection);
            MySqlDataAdapter Da = new MySqlDataAdapter(Cmd);
            Da.Fill(Dt);
            if (Dt.Rows.Count > 0) 
            {
               label.Text = Dt.Rows[0][COl].ToString(); 
            }
        }
